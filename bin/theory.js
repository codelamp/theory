...
theory = t = {
  ...
  method: function(desc, context, attribute){
    ...
    /// expose this to allow for other clever things later
    /// true, the may slow down newer browser environments that
    /// don't like you to change objects from their base class
    /// but JavaScript is made to be sketchable.
    //method.description = function(){ console.log('ABC'); return desc; }; <<<<<<<<-----------
    return method;
  },
  /*
   *
...
/**
 * Simplistic, readable but most of all quick overloading.
 *
 * ##### Inner workings
 *
...
t.overload.allCombinations = function(bits, options){
  if ( !options ) { options = {}; }
 ...
 *
 * @TODO: in order for arg names to work with optional args this 
 * code should handle multiple square bracket groups
 */
t.overload.parseKey = t.wrapper(function(wrapper){
  wrapper.keyPattern = /\[([^\]]+)\]/ig;
  wrapper.itemPattern = /, */g
  return function(key){
    var items = [];
    key.replace(wrapper.keyPattern, function(outer, inner){
      var bits = inner.split(wrapper.itemPattern);
...
/// @TODO: this replaces the above, and supports optional params
t.overload.parseKey = t.method({
  attributes: {
    keyPattern: /\[([^\]]+)\]/ig,
    itmPattern: /, */ig
  },
 ...
 */
t.overload.callItem = function(desc, item, context, args, hops){
 ...
 * be extended. These should make changes to the description object
 * before being handed to the item processors.
 *
 ...
 */
t.overload.descProcessors = new Array();
 ...
 * You can use:
 *
 *  e.g. [array:list, callable:callback, number:delay]
 *
 ...
 */
t.overload.descProcessors.inlineNames = t.wrapper(function(wrapper){
  wrapper.translatePattern = t.ExpReg(/([^\[\]\:\|\s,]+)\:([^\[\]\:\|\s,]+)/);
  return function(desc, internal){
 ...
 */
t.overload.descProcessors.translateToArgsObject = t.wrapper(function(wrapper){
  wrapper.sepPattern = / *> */,
 ...
 *
 *  e.g. [array, object|string]
 */
t.overload.descProcessors.alternativeKeys = function(desc, internal){
 ...
 *  e.g. [array, object] [string]?
 */
t.overload.descProcessors.optionalArguments = t.wrapper(function(wrapper){
